...
patterns.forEach(pattern => {
  ...
  files.forEach(rel => {
    ...
    const debuggerRe = /\bdebugger;?/g;

    const beforeConsole = (content.match(consoleRe) || []).length;
    const beforeDebugger = (content.match(debuggerRe) || []).length;

    content = content.replace(consoleRe, '');
    content = content.replace(debuggerRe, '');
    ...
    if (removed > 0 && content !== original) {
      ...
      console.log(`Edited ${rel}: removed ${removed} diagnostic statements`);
    }
  });
});
...
console.log(`Total diagnostic statements removed: ${totalRemoved}`);
if (changedFiles.length) console.log('Changed files:');
changedFiles.forEach(f => console.log(` - ${f.file}: ${f.removed}`));
